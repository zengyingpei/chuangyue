<template>
  <div class="box">
    <div class="aside">
      <div class="group">
        <div class="item" v-for="item in lists" :key="item.id" :style="select_user == item.userId ? 'color: red' : 'color:white'"  @click="selectUser(item.userId, item.id)">
          {{item.username}}
        </div>
      </div>
    </div>
    <div class="main">
        <div class="chat_box">
          ...
          <div class="chat_input">
            <textarea style="width: 90vw;height:16vh" name="" id="" cols="" rows="" v-model="input"></textarea>
          </div>
          <div class="chat_operate">
            <button type="button" class="btn btn-primary" id="submit" @click="send">发送</button>
          </div>
        </div>
    </div>
  </div>
    ...
    setup:()=> {
      ...
      const getTime=()=>{
        var date = new Date();
				...
				var now = year + '-' + addZero(month) + '-' + addZero(day) + ' ' + addZero(hour) + ":" +addZero(minute) + ':' + addZero(second);
				return now;
      }
